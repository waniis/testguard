<?php
$scratch = get_field('section_scratch');
if( $scratch ): ?>

<canvas id="scratch" width="100%" height="100%" 
  data-after-url="<?= $scratch['image_after'] ? $scratch['image_after'] : ''; ?>" 
  data-after-url-mobile="<?= array_key_exists( 'image_after_mobile', $scratch ) ? $scratch['image_after_mobile'] : '' ?>" 
  data-before-url="<?= $scratch['image_before'] ? $scratch['image_before'] : '' ?>" 
  data-before-url-mobile="<?= array_key_exists( 'image_before_mobile', $scratch ) ? $scratch['image_before_mobile'] : '' ?>" >
</canvas>

<div class="scratch-txt-container">
    <h2 class="scratch-title"><?php echo esc_html( $scratch['scratch_text']); ?>
    </h2>
    <a href="<?php the_field('catalogue_particulier', 'option'); ?>" class="btn-arrow-white w-inline-block">
      ...
      <div class="btn-text"><?php _e('Découvrir les solutions', 'guard-industrie') ?></div>
    </a>
